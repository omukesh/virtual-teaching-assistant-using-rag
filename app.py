...
class KnowledgeBaseManager:
    def __init__(self, knowledge_base_path='knowledge_base.json', embedding_model_name='all-MiniLM-L6-v2', vector_dimension=384):  # vector_dimension depends on embedding model
        ...
        self.documents = []  # Keep the original document for retrieval
        self.build_index() # Call build index initially
    ...
    def build_index(self):
        ...
        self.documents = [] # reset the documents for new build
        ids = []
        id_counter = 0
        self.metadata = []  # Metadata list
            ...
            if isinstance(entries, dict):  # Check if entries is a dictionary
                for topic, topic_entries in entries.items():
                    for entry in topic_entries:
                        if 'content' in entry:
                            self.documents.append(entry['content'])  # Use the 'content' field, Keep the document for the metadata lookup later
                            ids.append(id_counter)
                            self.metadata.append({"topic": topic, "category": category, "image_path": entry.get("image_path"), "generate_image": entry.get("generate_image")})  # Store metadata
                            id_counter += 1
        ...
        logging.info(f"FAISS index built with {len(self.documents)} documents.")

    def search(self, query, top_k=3):
        """Searches the FAISS index for the most relevant documents and retrieves metadata."""
        query_embedding = self.model.encode([query])
        D, I = self.index.search(query_embedding, np.array([top_k]).astype('int32'))  # D: distances, I: indices
        results = []
        for i in range(len(I[0])):
            ...
            if idx != -1:  # Check if valid
                ...
                results.append({
                    "content": self.documents[idx],  # the "content" associated to the metadata
                    "metadata": metadata,
                    "distance": D[0][i]
                })
        return results
...
class ImageGenerator:
    def __init__(self, model_id="runwayml/stable-diffusion-v1-5", device="cuda" if torch.cuda.is_available() else "cpu"): #or cuda if GPU available
        self.device = device
        self.pipe = StableDiffusionPipeline.from_pretrained(model_id, torch_dtype=torch.float16 if device=="cuda" else torch.float32) # Reduced precision for memory if on CUDA
        self.pipe = self.pipe.to(self.device)
        self.num_inference_steps = 25 #Set steps for image inference (25 as best number for performance and accuracy)
    ...
    def save_image(self, image, filename="generated_image.png"):
        ...
        try:
            image.save(os.path.join(UPLOAD_FOLDER, filename)) # Save in the uploads folder
            return os.path.join(UPLOAD_FOLDER, filename) #Return the file path.
        except Exception as e:
            logging.error(f"Error saving image: {e}")
            return None

# Main Function for Processing User Queries
def process_user_query(nlp, assistant, kb_manager, query, image_generator, is_voice=False, image_path=None, file_path=None):
    ...
    if results:
        ...
        if image_path:
            ...
            generate_image_prompt = metadata.get("generate_image")
            if generate_image_prompt:
              generated_image = image_generator.generate_image(generate_image_prompt)
              if generated_image:
                image_filename = f"generated_{int(time.time())}.png"
                saved_image_path = image_generator.save_image(generated_image, image_filename)
                if saved_image_path:
                  answer += f"\n\nHere's a generated visualization:\n![Generated Image]({saved_image_path})"
...
# Initialize image generator outside of the process request to avoid re-initializing
...
def process_request():
    try:
        # image_generator = ImageGenerator() #Initialization
        if 'file' in request.files:
            ...
            for file in files:
                if file and allowed_file(file.filename):
                    ...
                    if file_path.lower().endswith(('.png', '.jpg', '.jpeg')):
                        ...
                        if image_text:
                            answer, pdf_file = process_user_query(nlp, assistant, kb_manager, query=image_text, image_generator = image_generator, image_path=file_path)  # Now pass image_path
                        else:
                            logging.warning(f"OCR failed for image: {filename}")
                            return jsonify({'answer': "I couldn't extract text from the image. Please try a clearer image."}, 400)
                    # PDF Processing
                        ...
                        if pdf_text:
                            ...
                            kb_manager.build_index() # Rebuild FAISS when new entry is added

                    else:
                        logging.warning(f"Unsupported file type: {filename}")
                        return jsonify({'answer': "Unsupported file type. Only images and PDFs are supported."}, 400)
            ...
            if input_type == 'voice':
                ...
                if text:
                    answer, pdf_file = process_user_query(nlp, assistant, kb_manager, query=text, image_generator = image_generator)
                else:
                    return jsonify({'answer': "Could not understand speech"}, 400)
            elif query:
                answer, pdf_file = process_user_query(nlp, assistant, kb_manager, query=query,image_generator = image_generator)
            else:
        ...
        return jsonify({'answer': f"An error occurred: {str(e)}"}, 500)  # Include error message
